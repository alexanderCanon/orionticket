...
const GATEWAY_URL = "https://api.orionticket.com/v1"; // Fetching catalog through gateway
const SEATING_URL = "http://100.118.215.30:8083/v1"; // Direct to Seating Service to avoid gateway overhead
...
export function setup() {
    ...
    const seatsRes = http.get(
        `${SEATING_URL}/events/${eventId}/dates/${dateId}/seats`,
    );
...
export default function (data) {
    ...
    const randomSeat =
        data.seats[Math.floor(Math.random() * data.seats.length)];
    ...
    check(res, {
        ...
        "status is 409 (Conflict - Overbooking Prevented)": (r) =>
            r.status === 409,
